...
class Neo4jConnector():
    ...
    def add_course_if_not_exists(self, name, description, taught_on_semester, is_elective=False):
        if self.get_course(name, taught_on_semester, is_elective) is None:
            return Course(name=name, description=description, taught_on_semester=taught_on_semester, is_elective=is_elective).save()
        else:
    ...
    def get_field_of_study(self, name, start_years, faculty):
        results, meta = db.cypher_query(
            'MATCH (fieldOfStudy:FieldOfStudy) WHERE (fieldOfStudy.name=\"'+name+'\"AND fieldOfStudy.start_years=\"'+str(start_years)+'\" AND fieldOfStudy.faculty=\"'+faculty+'\") RETURN fieldOfStudy')
    ...
    def get_course(self, name, taught_on_semester, is_elective):
        ...
        results, meta = db.cypher_query(
            'MATCH (course:Course) WHERE (course.name=\"'+name+'\" AND course.taught_on_semester='+str(taught_on_semester)+' AND course.is_elective='+str(is_elective)+') RETURN course')
    ...
    def get_past_courses_without_electives(self, student):
        results, meta = db.cypher_query(
            'MATCH (s:Student)-[r1:STUDIES]->(f:FieldOfStudy)<-[r2:HAS]-(c:Course) WHERE (c.taught_on_semester<=s.is_on_semester AND s.index_number='+str(student.index_number)+' AND c.is_elective==False ) RETURN c')
        courses = [Course.inflate(row[0]) for row in results]
        return courses

    def get_past_courses_only_electives(self, student):
        results, meta = db.cypher_query(
            'MATCH (s:Student)-[r1:STUDIES]->(f:FieldOfStudy)<-[r2:HAS]-(c:Course) WHERE (c.taught_on_semester<=s.is_on_semester AND s.index_number='+str(student.index_number)+' AND c.is_elective==True ) RETURN c')
        courses = [Course.inflate(row[0]) for row in results]
        return courses
    ...
    def get_all_fields_of_study_for_student(self, student):
        results, meta = db.cypher_query(
            'MATCH (s:Student)-[r:STUDIES]->(f:FieldOfStudy) WHERE s.index_number='+str(student.index_number)+' RETURN f')
        field_of_study = [FieldOfStudy.inflate(row[0]) for row in results]
    ...
    def get_student_on_semester_for_fieldofstudy(self, student, fieldofstudy):
        results, meta = db.cypher_query(
            'MATCH (s:Student)-[r:STUDIES]->(f:FieldOfStudy) WHERE (s.index_number='+str(student.index_number)+' and f.name=\"'+fieldofstudy.name+'\" and f.faculty=\"'+fieldofstudy.faculty+'\" and f.start_years=\"'+fieldofstudy.start_years+'\") RETURN r.on_semester')
    ...
    def get_top_n_keywords_describing_course(self, course, n):
        results, meta = db.cypher_query(
            'MATCH (k:Keyword)-[r:DESCRIBES]->(c:Course) WHERE c.name=\"'+course.name+'\" RETURN k ORDER BY r.rating DESC LIMIT '+str(n))
        keywords = [Keyword.inflate(row[0]) for row in results]
        return keywords
